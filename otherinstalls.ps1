...
# Software list with VC++ AIO removed
$software = @(
    ...
    [PSCustomObject]@{ Name = '8. AltDrag'; Url = 'https://github.com/stefansundin/altdrag/releases/download/v1.1/AltDrag-1.1.exe' },
    [PSCustomObject]@{ Name = '9. Cursor'; Url = 'https://downloads.cursor.com/production/7801a556824585b7f2721900066bc87c4a09b743/win32/x64/user-setup/CursorUserSetup-x64-0.48.8.exe' },
    [PSCustomObject]@{ Name = '10. NVIDIA Broadcast'; Url = 'https://international.download.nvidia.com/Windows/broadcast/2.0.1/NVIDIA_Broadcast_v2.0.1.25267890.exe' }
)
...
foreach ($index in $selected) {
    if ($index -ge 0 -and $index -lt $software.Count) {
        ...
        try {
            ...
            if ($item.Name -match '1\. Nvidia Driver|2\. AMD Adrenalin|5\. FishStrap|10\. NVIDIA Broadcast') {
